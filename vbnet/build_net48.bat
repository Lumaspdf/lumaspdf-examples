...
rem Build every VB.Net example for .NET Framework 4.8 with the in-box vbc.exe.
rem Same structure as ..\csharp\build_net48.bat (see there): example + wrapper
rem source compiled together, x64 engine DLL staged next to the exe.
setlocal enabledelayedexpansion
set "VBC=%WINDIR%\Microsoft.NET\Framework64\v4.0.30319\vbc.exe"
if not exist "%VBC%" (echo .NET Framework 4.x vbc.exe not found & exit /b 1)
if exist "%~dp0..\..\include\Visual_Basic_Net\LumasPdf.vb" (
  set "WRAP=%~dp0..\..\include\Visual_Basic_Net\LumasPdf.vb"
  set "ENGINE=%~dp0..\..\win64\LumasPdf.dll"
) else (
  set "WRAP=%~dp0..\..\wrappers\vbnet\LumasPdf.vb"
...
for /D %%d in ("%~dp0*") do (
  for %%s in ("%%d\*.vb") do (
    echo === %%~ns
    "%VBC%" /nologo /platform:x64 /out:"%%d\%%~ns.exe" "%%s" "%WRAP%" >nul
...
echo ALL VB.Net EXAMPLES BUILT (net48 x64)
