...
// ===========================================================================
//  LumasReport example 10 -- Grouping + running aggregates
//  Covers: groupheader / detail / groupfooter / summary bands driven by a
//  grouped CSV (group="d.Cat"), plus the inline aggregate functions surfaced
//  in {{expr:}}. The engine maps these to TRptAggKind =
//    (agSum, agMin, agMax, agAvg, agCount, agFirst, agLast, agDistinct)
//  and folds them as a single streaming pass; a group footer sees exactly its
//  group's rows (subtotal), the summary sees the whole report (grand total).
//  Aggregate call forms:
//    SUM(x) AVG(x) COUNT(x) COUNT() MIN(x) MAX(x) FIRST(x) LAST(x)
//    COUNTDISTINCT(x)   -- MIN/MAX stay scalar when given >=2 args.
// ===========================================================================
{$APPTYPE CONSOLE}
uses
...
begin
  Result :=
    ...
    // group footer: subtotal + count + average, scoped to the Cat group
