# LumasPDF — Mirrored LumasPDF Delphi Examples

This folder mirrors the complete LumasPDF `examples/Delphi` set, running
against **LumasPdf.dll** instead of LumasPdf.dll.

## Layout

- `include\LumasPdfApi.pas` — the flat-API Delphi import unit (unit
  `LumasPdfApi`; TPDF class and all flat `pdf*`/`tbl*`/`ras*`/… functions),
  derived from the LumasPDF Delphi import unit with its library binding
  changed to `LumasPdf.dll` (both the static `LUMASPDFLIB` constant and the
...
- `..\test_files\` — the LumasPDF sample assets referenced by some examples
...
## Source changes vs the LumasPDF originals

1. `include\LumasPdfApi.pas` (was `LumasPdf.pas`): unit renamed to
   `LumasPdfApi`, DLL binding changed to `LumasPdf.dll`.
2. Every `.dpr`/`.pas`: `uses LumasPdf` / `lumaspdf in '...'` clauses
