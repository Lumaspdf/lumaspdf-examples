//  LumasReport example 12 -- Custom expression function (C# port)
//  Registers a managed expression function on the engine via rptRegisterFunction.
//  The callback matches the engine's C user-function ABI (stdcall):
//     int Fn(IntPtr User, IntPtr Args, int NArgs, IntPtr ResultV)  // 0=ok
//  TRptCValue is { Kind:Int32; B:Int32; I:Int64; F:Double; S:PAnsiChar },
//  ordinals vkNull=0 vkBool=1 vkInt=2 vkFloat=3 vkDate=4 vkStr=5. A string result
//  must stay valid until the next call, so we keep it in a static buffer.
//  Invoked from the report via {{expr: GREET('World')}}.
using System;
using System.IO;
using System.Runtime.InteropServices;
using LumasPdfSdk;

class Ex12
{
    const string PDF_DEMO_KEY = "LUMAS-LumasReportExamples-DD5D40E0";
    const string RPT_DEMO_KEY = "LRPT2-TFIwMgECAAIAAgIAAQADBAD___9_BAEAAAUBAAAGCAAAAAAAAAAAAAcQAEx1bWFzUnB0RXhhbXBsZXM.sneCg6LX_J59AcqzUZPx1WqQ7jGQZyji4z0WMm_vIoycNySw12vGVoKV76gk3-yQtx3LfZZepGxoDdQAgf5MAA";

    const int VK_INT = 2;
    const int VK_STR = 5;
    // Field offsets in TRptCValue: Kind@0, B@4, I@8, F@16, S@24
    const int OFF_I = 8;
    const int OFF_S = 24;

    [UnmanagedFunctionPointer(CallingConvention.StdCall)]
    delegate int TRptUserFn(IntPtr User, IntPtr Args, int NArgs, IntPtr ResultV);

    // Keep the delegate alive so the GC does not collect the thunk.
    static TRptUserFn _greetCb = GreetFn;
    // Backing store for the string handed back to the engine. Must outlive the
    // callback return (engine copies it immediately on the way out).
    static IntPtr _gResult = IntPtr.Zero;

    static int GreetFn(IntPtr user, IntPtr args, int nArgs, IntPtr resultV)
    {
        if (nArgs != 1 || args == IntPtr.Zero || resultV == IntPtr.Zero) return -1;
        int kind = Marshal.ReadInt32(args, 0);
        string argStr = "";
        if (kind == VK_STR)
        {
            IntPtr sptr = Marshal.ReadIntPtr(args, OFF_S);
            if (sptr != IntPtr.Zero) argStr = Marshal.PtrToStringAnsi(sptr);
        }
        else if (kind == VK_INT)
        {
            long v = Marshal.ReadInt64(args, OFF_I);
            argStr = v.ToString();
        }
        else return -2;

        if (_gResult != IntPtr.Zero) Marshal.FreeHGlobal(_gResult);
        _gResult = Marshal.StringToHGlobalAnsi("Hello, " + argStr + "!");
        Marshal.WriteInt32(resultV, 0, VK_STR);
        Marshal.WriteIntPtr(resultV, OFF_S, _gResult);
        return 0;
    }

    static string BuildXml()
    {
        string s = "";
        s += "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
        s += "<report name=\"CustomFn\" tagLangVersion=\"1\">\n";
        s += " <page width=\"210\" height=\"297\" marginLeft=\"15\" marginTop=\"15\" marginRight=\"15\" marginBottom=\"15\"/>\n";
        s += " <bands>\n";
        s += "  <band kind=\"reportheader\" name=\"rh\" height=\"24\">\n";
        s += "   <text name=\"g1\" x=\"0\" y=\"0\"  w=\"180\" h=\"8\" fontSize=\"16\">{{expr: GREET('World') }}</text>\n";
        s += "   <text name=\"g2\" x=\"0\" y=\"10\" w=\"180\" h=\"8\" fontSize=\"12\">{{expr: GREET('LumasReport') }}</text>\n";
        s += "  </band>\n";
        s += " </bands>\n";
        s += "</report>\n";
        return s;
    }

    static void Main()
    {
        IntPtr pdf, eng;
        if (!BootEngine(out pdf, out eng)) return;

        IntPtr fnPtr = Marshal.GetFunctionPointerForDelegate(_greetCb);
        if (!LumasPdf.rptRegisterFunction(eng, "GREET", 1, 1, fnPtr, IntPtr.Zero))
        {
            Console.WriteLine("rptRegisterFunction failed"); DumpRptError(eng); goto Cleanup;
        }
        Console.WriteLine("registered custom function GREET/1");

        string dir = AppDomain.CurrentDomain.BaseDirectory;
        string lrpt = Path.Combine(dir, "12_custom_function.lrpt");
        string outPdf = Path.Combine(dir, "12_custom_function.pdf");
        string outTxt = Path.Combine(dir, "12_custom_function.txt");
        File.WriteAllText(lrpt, BuildXml());

        IntPtr job = LumasPdf.rptOpenReportA(eng, lrpt);
        if (job == IntPtr.Zero) { Console.WriteLine("open failed"); DumpRptError(eng); goto Cleanup; }
        if (!LumasPdf.rptRender(job)) { Console.WriteLine("render failed"); DumpRptError(eng); goto CloseJob; }
        if (!LumasPdf.rptExportA(job, LumasPdfConsts.RPT_EXP_PDF, outPdf)) { Console.WriteLine("export PDF failed"); DumpRptError(eng); goto CloseJob; }
        if (!LumasPdf.rptExportA(job, LumasPdfConsts.RPT_EXP_TEXT, outTxt)) { Console.WriteLine("export TEXT failed"); DumpRptError(eng); goto CloseJob; }
        Console.WriteLine("wrote " + outPdf);
        Console.WriteLine("OK");
    CloseJob:
        LumasPdf.rptCloseReport(job);
    Cleanup:
        LumasPdf.rptDeleteEngine(eng);
        LumasPdf.pdfDeletePDF(pdf);
    }

    static bool BootEngine(out IntPtr pdf, out IntPtr eng)
    {
        eng = IntPtr.Zero;
        pdf = LumasPdf.pdfNewPDF();
        if (pdf == IntPtr.Zero) { Console.WriteLine("pdfNewPDF failed"); return false; }
        LumasPdf.pdfSetLicenseKey(pdf, PDF_DEMO_KEY);
        LumasPdf.rptSetRptLicenseKeyA(pdf, RPT_DEMO_KEY);
        eng = LumasPdf.rptCreateEngineA(pdf, null);
        if (eng == IntPtr.Zero) { Console.WriteLine("rptCreateEngine failed"); return false; }
        return true;
    }

    static void DumpRptError(IntPtr eng)
    {
        IntPtr p = Marshal.AllocHGlobal(Marshal.SizeOf(typeof(TRptErrorInfoC)));
        try
        {
            if (LumasPdf.rptGetLastError(eng, p))
            {
                var info = (TRptErrorInfoC)Marshal.PtrToStructure(p, typeof(TRptErrorInfoC));
                if (info.Code != 0)
                    Console.WriteLine("  ! rpt error " + info.Code + " [" + info.Module_ + "] at " + info.Location + ": " + info.Msg);
            }
        }
        finally { Marshal.FreeHGlobal(p); }
    }
}
