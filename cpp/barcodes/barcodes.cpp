...
// The Delphi original declares BarcodeName as AnsiString but passes it to
// TPDF.WriteFTextEx(...: String), so the compiler widens it and the call binds
// to the WIDE overload; SetFont / CreateNewPDF / OpenOutputFile likewise take
// Delphi `string`. LWCHAR is wchar_t on Windows and char16_t elsewhere, so
// literals must use the header's own LUMAS_TEXT() macro -- a bare u"..." does
// not convert to LWCHAR* under MSVC and a bare L"..." is 4 bytes wide off
// Windows. WS() reproduces the AnsiString -> UnicodeString widening the Delphi
// compiler inserts for the barcode names (all pure ASCII).
...
int main(int argc, char** argv){
    ...
    while(i < cnt){
        pdfAppend(pdf);
        pdfSetFontW(pdf, W_("Helvetica"), fsRegular, 6.5, 1, cp1252);
        ...
        for(int yy = 1; yy <= ny; ++yy){
            ...
            for(int xx = 1; xx <= nx; ++xx){
                ...
                pdfWriteFTextExW(pdf, x, y - 10.0, w, -1.0, taCenter, (LWCHAR*)WS(TEST_CODES[i].BarcodeName).c_str());
