...
//      marks the state annot as needing a /Popup. Delphi's counterpart in
...
//      state annots now matches the reference while /T /Subj /Contents on the
...
int main(int argc, char** argv){
    ...
    SI32 reply = pdfSetAnnotMigrationStateW(pdf, annot, asCompleted, W_("Harry"));
    pdfSetAnnotStringW(pdf, reply, asContent, W_("The state was set to Completed!"));

    reply = pdfSetAnnotMigrationStateW(pdf, reply, asAccepted, W_("Jim"));
    pdfSetAnnotStringW(pdf, reply, asContent, W_("The state was set to Accepted!"));
    pdfEndPage(pdf);
