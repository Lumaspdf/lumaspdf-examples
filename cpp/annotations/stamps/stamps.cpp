...
// The Delphi original passes Delphi `string` (UnicodeString), so StampAnnot
...
// pdfSetLanguage has no W twin (it takes an ASCII ISO tag), so it stays as is.
...
int main(int argc, char** argv){
    ...
    SI32 a = pdfStampAnnotW(pdf, rsApproved, 135.0, 50.0, 300.0, 10.0, W_("Test app"), W_("Stamp Annotations"), W_("The default language is English!"));
    pdfSetAnnotColor(pdf, a, fcBorderColor, csDeviceRGB, RGB_(120, 190, 92));

    pdfSetLanguage(pdf, "DE");
    a = pdfStampAnnotW(pdf, rsApproved, 135.0, 150.0, 300.0, 10.0, W_("Test app"), W_("Stamp Annotations"), W_("The same stamp in German!"));
    pdfSetAnnotColor(pdf, a, fcBorderColor, csDeviceRGB, RGB_(230, 65, 132));

    pdfSetLanguage(pdf, "FR");
    a = pdfStampAnnotW(pdf, rsApproved, 135.0, 250.0, 300.0, 10.0, W_("Test app"), W_("Stamp Annotations"), W_("The same stamp in French!"));
