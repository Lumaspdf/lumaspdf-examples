...
// U+00B7 MIDDLE DOT, encoded UTF-8 (0xC2 0xB7).
//
// This string goes into the .lrpt below, whose XML declaration says
// encoding="UTF-8" -- so it MUST be UTF-8, not a raw Latin-1 code unit. The
// original port copied VB6's ChrW(183)-through-the-ANSI-codepage byte and
// emitted a bare 0xB7, which is not a legal UTF-8 sequence: the report parser
// replaced each one with U+FFFD, and every text-bearing export (.txt/.html/
// .svg) carried EF BF BD where the Delphi reference carries C2 B7. It also
// perturbed the embedded subset font, because U+FFFD needs a different glyph
// than the middle dot.
