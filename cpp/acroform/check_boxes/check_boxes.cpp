...
int main(int argc, char** argv){
    ...
    pdfWriteTextW(pdf, 50.0, 50.0, (LWCHAR*)LUMAS_TEXT("Normal check boxes."));
    ...
    pdfWriteTextW(pdf, 50.0, 100.0, (LWCHAR*)LUMAS_TEXT("Field group with check boxes."));
    ...
    pdfChangeFontSize(pdf, 10.0);
    pdfWriteFTextExW(pdf, 50.0, 150.0, 220.0, -1.0, taLeft, (LWCHAR*)LUMAS_TEXT("This group works like a radio button but only radio buttons get a round border if the check box character is set to ccCircle. No problem, set the border width to zero and draw the circle in background if needed."));
    ...
    pdfChangeFontSize(pdf, 10.0);
    pdfWriteFTextExW(pdf, 300.0, 50.0, 250.0, -1.0, taLeft, (LWCHAR*)LUMAS_TEXT("This is a radio button. Since Acrobat 7 it is no longer possible to deselect the active check box, except with a reset form or Javascript action."));
    ...
    pdfChangeFontSize(pdf, 10.0);
    f = pdfCreateButtonW(pdf, "Reset", (LWCHAR*)LUMAS_TEXT("Reset"), -1, 400.0, y, 60.0, 20.0);
    ...
    y = y + 40.0;
    pdfChangeFontSize(pdf, 10.0);
    pdfWriteFTextExW(pdf, 300.0, y, 250.0, -1.0, taLeft, (LWCHAR*)LUMAS_TEXT("The RadioIsUnion flag has only an effect if at least two check boxes use the same export value."));
    y = pdfGetPageHeight(pdf) - pdfGetLastTextPosY(pdf) + 10.0;
