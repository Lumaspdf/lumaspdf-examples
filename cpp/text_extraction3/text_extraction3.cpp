...
static void WritePageIdentifier(SI32 PageNum){
    wchar_t buf[128];
    if(PageNum > 1) fwrite(L"\r\n", sizeof(wchar_t), 2, g_File);
    int n = swprintf(buf, 128, L"%%----------------------- Page %d -----------------------------\r\n", (int)PageNum);
    if(n > 0) fwrite(buf, sizeof(wchar_t), (size_t)n, g_File);
}
...
int main(){
    ...
    for(SI32 i = 1; i <= cnt; i++){
        ...
        if(pdfExtractText(pdf, (UI32)i, (TTextExtractionFlags)(tefDeleteOverlappingText | tefSortTextX), nullptr, &textPtr, &textLen) != 0){
            if(textLen > 0 && textPtr) fwrite(textPtr, sizeof(wchar_t), textLen, g_File);
        }
    }
    fclose(g_File);
