...
// AddOutputIntent/OpenOutputFile. CreateSigField and CloseAndSignFile have ONLY
// AnsiString overloads in dynapdf.pas, so those two stay on the A entry points.
...
int main(){
    ...
    pdfAppend(pdf);
    pdfSetFontW(pdf, (LWCHAR*)LUMAS_TEXT("Arial"), fsNone, 10.0, 1, cp1252);
    LWCHAR* body = (LWCHAR*)LUMAS_TEXT(
        ...
        "returns 0, DynaPDF assumes that the prior changes were accepted and processing continues.\r\r"
    ...
    pdfWriteFTextExW(pdf, 50.0, 45.0, 150.0, -1.0, taCenter, (LWCHAR*)LUMAS_TEXT("DynaPDF"));
    ...
    switch(pdfCheckConformance(pdf, ctPDFA_1b_2005, 0, 0, 0, 0)){
        case 1:
        case 3: pdfAddOutputIntentW(pdf, (LWCHAR*)LUMAS_TEXT("sRGB.icc")); break;             // Gray, RGB
        case 2: pdfAddOutputIntentW(pdf, (LWCHAR*)LUMAS_TEXT("ISOcoated_v2_bas.ICC")); break; // CMYK
    }
