...
// ------------------------- output helpers -------------------------
static void WriteWStr(const wchar_t* s){ fwrite(s, sizeof(wchar_t), wcslen(s), m_File); }
static void WriteWCharsFromPtr(const LWCHAR* p, SI32 count){
    if(!p || count <= 0) return;
    fwrite(p, sizeof(wchar_t), (size_t)count, m_File);
}
...
int main(){
    ...
    for(SI32 i = 1; i <= pdfGetPageCount(m_PDF); i++){
        pdfEditPage(m_PDF, i);
        wchar_t buf[128];
        int n = swprintf(buf, 128, L"%ls%%----------------------- Page %d -----------------------------\r\n",
                         (i > 1 ? L"\r\n" : L""), (int)i);
        if(n > 0) fwrite(buf, sizeof(wchar_t), (size_t)n, m_File);
        ParsePage();
        pdfEndPage(m_PDF);
    }
    fclose(m_File);
