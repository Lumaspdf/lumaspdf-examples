...
// Signs license.PDF four times using incremental updates.
{$APPTYPE CONSOLE}
uses
...
function SignFile(const InFileName, OutFileName, FieldName, Reason: AnsiString;
...
begin
  ...
  try
    ...
    if SignFile('../../../../license.PDF', fp, 'Signature1', 'Test signature 1', 50.0, True) then
      if SignFile(fp, fp, 'Signature2', 'Test signature 2', 430.0, True) then
        if SignFile(fp, fp, '', 'Test signature 3', 0.0, False) then
          if SignFile(fp, fp, '', 'Test signature 4', 0.0, False) then
