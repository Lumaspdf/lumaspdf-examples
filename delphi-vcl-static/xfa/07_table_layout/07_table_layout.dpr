program table_layout_vcl;
{$APPTYPE CONSOLE}
(*
  VCL-static-component-flavour port of examples\delphi\xfa\07_table_layout
  (flavor-tour example 7 of 10: layout="table" -- a 4-column "Product
  Comparison Table" with a header row + 5 data rows, columnWidths-driven,
  every cell carrying its own <para hAlign> so left/right/center are all
  exercised).
  ...
  No XML parsing here: 07_table_layout.template.xml / .datasets.xml are the
...
begin
  ...
  R1 := RenderExample(
    ExeDir + '07_table_layout.template.xml',
    ExeDir + '07_table_layout.datasets.xml',
    ExeDir + 'output.pdf');
