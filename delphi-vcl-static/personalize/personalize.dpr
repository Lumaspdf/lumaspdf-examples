...
// Imports a tax form, fills the fields, adds a web link. Wrap.Static MUST be first.
{$APPTYPE CONSOLE}
uses
  Lumas.Pdf.Wrap.Static,        // binds every engine entry point in-process
  System.SysUtils,
  Lumas.Pdf.Types,              // enums (viewer prefs, annot flags, highlight mode, fonts)
...
begin
  ...
  try
    ...
    pdf.SetViewerPreferences(Integer(vpDisplayDocTitle), Integer(avNone));
