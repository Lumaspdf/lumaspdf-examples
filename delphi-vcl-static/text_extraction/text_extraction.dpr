...
// Imports a PDF and extracts its text with GetPageText()/TPDFStack, rebuilding text
// lines and word boundaries by transforming each text record to user space.
...
uses
  ...
  System.Classes,
  System.Math,
  Lumas.Pdf.Types,              // TCTM, TTextRecordWPtr, lcm*, if*, ptOpen, aff*
  Lumas.Pdf.ApiTypes,           // TPDFStack
...
begin
  ...
  try
    ...
    if m_PDF.OpenImportFileA('in.pdf', Ord(ptOpen), '') < 0 then Exit;
