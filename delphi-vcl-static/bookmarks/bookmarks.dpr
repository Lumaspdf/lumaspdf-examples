...
// Logic mirrors examples\c\bookmarks\bookmarks.c (flat API -> Core methods).
{$APPTYPE CONSOLE}
uses
  Lumas.Pdf.Wrap.Static,        // MUST be first -- binds every engine entry point in-process
  System.SysUtils,
  Lumas.Pdf.Types,              // fsRegular/fsItalic/.., cp1252, dt*, ot*, oe*, hmInvert, pl*, pf*
...
begin
  ...
  try
    ...
    pdf.Append;
      ...
      pdf.WriteFTextExA(x, y, 90.0, -1, Ord(taCenter), 'We zoom into the rectangle');
    ...
    pdf.Append;
      pdf.ChangeFont(f);
      pdf.WriteFTextExA(50.0, 50.0, pdf.GetPageWidth - 100.0, -1.0, Ord(taLeft),
    ...
    pdf.SetPageLayout(Ord(plOneColumn));
