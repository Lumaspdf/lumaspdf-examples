program plugin;
// ===========================================================================
//  LumasReport PURE-VCL STATIC example 13 -- Register a custom function + custom
//  exporter DIRECTLY via native function pointers (PLUGIN-FREE: no
//  rpt_testplugin.dll, no rptLoadPlugin). Static build: engine linked in
//  (LUMAS_STATIC), NO LumasPdf.dll.
...
//  Export(Job, 100, '<dir>\13_custom.out').
// ===========================================================================
{$APPTYPE CONSOLE}
{$MINENUMSIZE 4}
{$ALIGN 8}
...
type
  // Engine's C user-function value ABI (declared locally for the static build).
...
begin
  ...
  try
    ...
    try
      ...
      if not EC.RegisterFunctionA('PlugDouble', 1, 1, @PlugDouble, nil) then
        begin Writeln('rptRegisterFunction failed'); DumpRptError(Pdf, Eng); Halt(2); end;
      if not EC.RegisterExporter(100, @PlugExport, nil) then
        begin Writeln('rptRegisterExporter failed'); DumpRptError(Pdf, Eng); Halt(2); end;
      ...
      try
        ...
        if not JC.ExportA(100, PAnsiChar(OutCustom)) then
          begin Writeln('custom export failed'); DumpRptError(Pdf, Eng); Halt(7); end;
