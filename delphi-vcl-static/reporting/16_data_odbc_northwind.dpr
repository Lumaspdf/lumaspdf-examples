...
// ===========================================================================
//  LumasReport PURE-VCL STATIC example 16 -- ODBC data provider over Northwind.mdb
//  Static build: engine linked in (LUMAS_STATIC), NO LumasPdf.dll. Covers the
//  "odbc" data provider, a live DB connection + JOIN + ORDER BY, grouping, field
//  interp. Uses the 64-bit "Microsoft Access Driver (*.mdb, *.accdb)".
// ===========================================================================
{$APPTYPE CONSOLE}
uses
  ...
  try
    ...
    try
      ...
      try
        ...
        Writeln(Format('rendered %d page(s) from Northwind.mdb (odbc)', [JC.GetPageCount]));
        if not JC.ExportA(RPT_EXP_PDF,  PAnsiChar(OutPdf)) then begin Writeln('pdf export failed'); DumpRptError(Pdf, Eng); Halt(4); end;
        if not JC.ExportA(RPT_EXP_TEXT, PAnsiChar(OutTxt)) then begin Writeln('text export failed'); DumpRptError(Pdf, Eng); Halt(5); end;
