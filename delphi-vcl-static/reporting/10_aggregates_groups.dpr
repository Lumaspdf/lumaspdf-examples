...
// ===========================================================================
//  LumasReport PURE-VCL STATIC example 10 -- Grouping + running aggregates
//  Static port of examples\delphi_oo\reporting\10_aggregates_groups.dpr:
//  the whole engine is linked into this exe (LUMAS_STATIC) -- NO LumasPdf.dll.
//  Boots via TLumasPDFCore + report CORE classes; drives a grouped CSV with
//  groupheader/detail/groupfooter/summary bands and the inline aggregates:
//    SUM(x) AVG(x) COUNT(x) COUNT() MIN(x) MAX(x) FIRST(x) LAST(x) COUNTDISTINCT(x)
// ===========================================================================
{$APPTYPE CONSOLE}
uses
  Lumas.Pdf.Wrap.Static,        // MUST be first -- binds every engine entry point
  System.SysUtils, System.Classes,
  Lumas.Rpt.Types,              // TRPT, TRPTJOB, RPT_EXP_*
  Lumas.Rpt.Errors,             // TRptErrorInfoC
    ...
    try
      ...
      try
        ...
        Writeln(Format('rendered %d page(s), grouped by Cat with per-group + grand totals',
          [JC.GetPageCount]));
        JC.ExportA(RPT_EXP_PDF,  PAnsiChar(OutPdf));
        JC.ExportA(RPT_EXP_TEXT, PAnsiChar(OutTxt));
