...
// ============================================================================
//  LumasReport PURE-VCL STATIC example 19 -- Build a .lrpt STEP-BY-STEP, bind it
//                               to the real Northwind.mdb, then SHOW it in the
//                               embedded viewer.
//  Static build: engine linked in (LUMAS_STATIC), NO LumasPdf.dll. The .lrpt is
//  assembled one labelled block at a time (STEP 1..14). Then: OpenReport ->
//  Render -> Export / Preview.
//  Preview BLOCKS until the viewer window is closed; pass --headless
...
begin
  ...
  begin Writeln('Northwind.mdb not found: ' + MDB); Halt(1); end;
  ...
  try
    ...
    try
      ...
      try
        JC.SetParamStr('Title',   'Northwind Product Catalog');
        JC.SetParamStr('Company', 'LumasPDF Trading Co.');
        ...
        if not JC.ExportA(RPT_EXP_PDF, PAnsiChar(OutPdf)) then
          begin Writeln('pdf export failed'); DumpRptError(Pdf, Eng); Halt(4); end;
        if not JC.ExportA(RPT_EXP_TEXT, PAnsiChar(OutTxt)) then
          begin Writeln('text export failed'); DumpRptError(Pdf, Eng); Halt(5); end;
        ...
        begin
          ...
          if not JC.PreviewA(PAnsiChar(AnsiString('Northwind Product Catalog'))) then
          begin
