...
// Imports a PDF, converts it to PDF/A-3b via CheckConformance with NATIVE font-not-found
// and ICC-replacement @-callbacks, then writes the result. Wrap.Static MUST be first.
{$APPTYPE CONSOLE}
uses
  Lumas.Pdf.Wrap.Static,        // binds every engine entry point in-process
  System.SysUtils,
  Lumas.Pdf.Types,              // enums (conformance, check options, ICC profile type)
  Lumas.Pdf.Wrap.Core,          // TLumasPDFCore (flat API as methods)
  Lumas.Pdf.Wrap.Imports;       // flat pdfReplaceFontA / pdfReplaceICCProfileA for the callbacks
...
// Data is the PDF handle (PPDF) passed as UserData to CheckConformance.
...
function ConvertFile(pdf: TLumasPDFCore; ConvType: Integer; const InFile, OutFile: AnsiString): Boolean;
var
  i, n, retval: Integer;
  convFlags: Cardinal;
  e: TPDFError;
begin
  ...
  retval := pdf.CheckConformance(ConvType, convFlags, pdf.InstanceHandle, @FontNotFoundProc, @ReplaceICCProfileProc);
  case retval of
...
begin
  ...
  try
    ...
    if ConvertFile(pdf, Ord(ctPDFA_3b), '../../../../sample_multipage.pdf', AnsiString(outFile)) then
