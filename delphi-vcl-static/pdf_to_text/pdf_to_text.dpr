...
// Imports a PDF and writes each page's text to out.txt. Wrap.Static MUST be first.
{$APPTYPE CONSOLE}
uses
  Lumas.Pdf.Wrap.Static,        // binds every engine entry point in-process
  System.SysUtils,
  Lumas.Pdf.Types,              // engine enums (import/cmap flags, pwd type)
  Lumas.Pdf.Wrap.Core;          // TLumasPDFCore (flat API as methods)

const
  emNoFuncNames = $10000000; // error-mode flag
