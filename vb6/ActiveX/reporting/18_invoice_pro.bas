...
Option Explicit
' ===========================================================================
'  ActiveX/COM mirror of examples\Vb6\reporting\18_invoice_pro.bas.
'  Same feature: a polished, print-ready invoice built from the banded model +
'  the SECTION-BOUNDED line feature (scope="section"): horizontal rules auto-
'  span the band WIDTH, vertical rules auto-span the band HEIGHT. Money
'  columns right-aligned; totals use inline SUM(Qty*Price). Exports to
'  PDF/HTML/SVG/TEXT + CSV/XLSX/XLS -- via the LumasPdf ActiveX component's
'  Rpt* methods.
' ===========================================================================
...
Const RPT_EXP_HTML As Long = 1
Const RPT_EXP_CSV As Long = 2
Const RPT_EXP_TEXT As Long = 5
Const RPT_EXP_SVG As Long = 6
Const RPT_EXP_XLSX As Long = 7
Const RPT_EXP_XLS As Long = 10
...
Private Sub ExportOne(ByVal Job As Long, ByVal Target As Long, ByVal Path As String)
    If pdf.RptExport(Job, Target, Path) <> 0 Then Debug.Print "  wrote " & Path Else Debug.Print "  EXPORT FAILED: " & Path
End Sub
...
Public Sub Main()
    ...
    Debug.Print "rendered " & pdf.RptGetPageCount(Job) & " page(s); exporting:"
    ExportOne Job, RPT_EXP_PDF, Dir_ & "18_invoice.pdf"
    ExportOne Job, RPT_EXP_HTML, Dir_ & "18_invoice.html"
    ExportOne Job, RPT_EXP_SVG, Dir_ & "18_invoice.svg"
    ExportOne Job, RPT_EXP_TEXT, Dir_ & "18_invoice.txt"
    ExportOne Job, RPT_EXP_CSV, Dir_ & "18_invoice.csv"
    ExportOne Job, RPT_EXP_XLSX, Dir_ & "18_invoice.xlsx"
    ExportOne Job, RPT_EXP_XLS, Dir_ & "18_invoice.xls"
CloseJob:
...
ErrHandler:
    ...
    MsgBox "Failed: " & Err.Description & extra, vbCritical, "18_invoice_pro (ActiveX)"
End Sub
