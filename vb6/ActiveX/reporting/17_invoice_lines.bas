...
Option Explicit
' ===========================================================================
'  ActiveX/COM mirror of examples\Vb6\reporting\17_invoice_lines.bas.
'  Same feature: an invoice exercising the full <line> surface (orient h/v/
'  free, scope band/section/page, hAlign/vAlign placement, dash solid/dot/
'  dash/dashdot, double, width/color/cap) plus inline aggregates
'  (SUM(Qty*Price)), exported to PDF, HTML, SVG, TEXT + native CSV, XLSX, XLS
'  -- via the LumasPdf ActiveX component's Rpt* methods.
' ===========================================================================
...
Private Sub ExportOne(ByVal Job As Long, ByVal Target As Long, ByVal Path As String)
    If pdf.RptExport(Job, Target, Path) <> 0 Then
...
Public Sub Main()
    ...
    Debug.Print "rendered " & pdf.RptGetPageCount(Job) & " page(s); exporting to 7 formats:"
    ' visual formats (lines rendered natively)
    ExportOne Job, RPT_EXP_PDF, Dir_ & "17_invoice.pdf"
    ExportOne Job, RPT_EXP_HTML, Dir_ & "17_invoice.html"
    ...
    ExportOne Job, RPT_EXP_CSV, Dir_ & "17_invoice.csv"
    ExportOne Job, RPT_EXP_XLSX, Dir_ & "17_invoice.xlsx"
    ExportOne Job, RPT_EXP_XLS, Dir_ & "17_invoice.xls"
CloseJob:
...
ErrHandler:
    ...
    MsgBox "Failed: " & Err.Description & extra, vbCritical, "17_invoice_lines (ActiveX)"
End Sub
