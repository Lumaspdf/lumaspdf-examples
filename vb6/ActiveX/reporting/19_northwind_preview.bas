...
Option Explicit
' ===========================================================================
'  ActiveX/COM mirror of examples\Vb6\reporting\19_northwind_preview.bas.
'  Same feature: build a .lrpt STEP-BY-STEP, bind it to the real
'  Northwind.mdb, render + export -- via the LumasPdf ActiveX component's
'  Rpt* methods. Assembles the .lrpt one labelled block at a time
'  (STEP 1..14) the way the engine reads it -- top to bottom -- then does the
'  four things every LumasReport job does:
'      BootEngine -> pdf.RptOpenReport -> pdf.RptRender -> pdf.RptExport.
'
'  The embedded preview (pdf.RptPreview, which renders to a temp PDF and pops
'  the SDK's built-in viewer window via vwrShowFileW -- it BLOCKS until the
'  window is closed) is included, same as the flat-DLL original: opened by
'  default, skipped when run with /headless or /no-preview so automated runs
'  stay unattended.
' ===========================================================================
...
' Uses the 32-bit legacy Jet ODBC driver -- see 16_data_odbc_northwind.bas for
' why (VB6 is a 32-bit process; the ActiveX component it loads is 32-bit too).
Private Function BuildReportXml() As String
    ...
    s = s & "    <style name=""SubL""    fontName=""Helvetica"" fontSize=""8.5"" bold=""1"" textColor=""005F3A1F""/>" & vbLf
    ...
    s = s & "      <text name=""sl"" x=""3""   y=""1.5"" w=""110"" h=""4"" style=""SubL"" wordWrap=""0"">Subtotal -- {{expr: d.CategoryName}} ({{expr: COUNT()}} products)</text>" & vbLf
...
' Was --headless / --no-preview passed on the command line? (mirror of the
' flat-DLL original / the Delphi .dpr)
...
Public Sub Main()
    ...
    pdf.RptSetParamStr Job, "Title", "Northwind Product Catalog"
    pdf.RptSetParamStr Job, "Company", "LumasPDF Trading Co."
    ...
    ' -- *** EMBEDDED PREVIEW *** ------------------------------------------------
    '  pdf.RptPreview renders the job to a temp PDF and opens the SDK's built-in
    '  viewer window; the call BLOCKS until the window is closed. Mirrors the
    '  original: pop by default, skip on --headless/--no-preview.
    If IsHeadless() Then
    ...
    Else
        ...
        If pdf.RptPreview(Job, "Northwind Product Catalog") = 0 Then
...
ErrHandler:
    ...
    MsgBox "Failed: " & Err.Description & extra, vbCritical, "19_northwind_preview (ActiveX)"
End Sub
