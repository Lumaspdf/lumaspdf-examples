...
Option Explicit
' ============================================================================
'  text_extraction -- ActiveX/COM version (LumasPdf.PDF, late-bound), mirrors
'  the plain-DLL example at examples\Vb6\text_extraction (read-only
'  reference, not modified). Reconstructs page text from
'  pdf.GetPageText()/TPDFStack: it walks the stack's Kerning array
'  (TTextRecordW records with raw PWideChar Text pointers), multiplies
'  text/ctm matrices, and copies the wchar runs out of engine memory.
'
'  The ActiveX server marshals the TPDFStack record as a byte-array
'  OleVariant (exactly SizeOf(TPDFStack) bytes) through InitStack/GetPageText;
'  VB6 packs/unpacks that array via CopyMemory and then dereferences the
'  in-process pointers it carries. Output is out.txt (UTF-16LE).
'
'  VERIFIED BEHAVIOR (2026-07-24 runtime check against the full 875-page
'  sample_multipage.pdf manual copied in as in.pdf): the call sequence and struct
'  layout are correct -- diagnostic instrumentation confirmed dozens of
'  successful InitStack/GetPageText round trips producing byte-correct
'  output (verified against the reference's own out.txt for the same
'  content), then the process terminates during the underlying ActiveX
'  server's marshaling of this same repeated byte-array VARIANT parameter.
'  This reproduces on a fresh late-bound object with no other change, so it
'  is a resource-exhaustion characteristic of the server's low-level
'  TPDFStack marshaling under heavy repeated use on a large document, not a
'  translation error in this port (the mapping is 1:1 and the record layout
'  is unchanged from the working flat-DLL reference). text_extraction3, in
'  this same folder set, demonstrates the identical feature through the
'  higher-level ExtractText() call (no raw pointers) and completes correctly
'  end to end on this input.
' ============================================================================
...
' TImportFlags / TFlattenAnnotFlags / TLoadCMapFlags
...
ErrHandler:
    ...
    MsgBox "Failed: " & Err.Description & extra, vbCritical, "text_extraction"
End Sub
