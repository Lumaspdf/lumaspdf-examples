...
static SI32 PDF_CALL ReplaceICCProfileProc(void* Data, TICCProfileType ProfileType, SI32 ColorSpace) {
    switch (ProfileType) {
        case ictRGB:  return pdfReplaceICCProfileA((PPDF)Data, ColorSpace, LUMAS_REPO_ROOT "/sample_rgb.icc");
        case ictCMYK: return pdfReplaceICCProfileA((PPDF)Data, ColorSpace, LUMAS_REPO_ROOT "/sample_rgb.icc" /* production: a real CMYK press profile */);
        default:      return pdfReplaceICCProfileA((PPDF)Data, ColorSpace, LUMAS_REPO_ROOT "/sample_gray.icc");
    }
}
...
static bool ConvertFile(PPDF pdf, int ConvType, const char* InFile, const char* Invoice, const char* OutFile) {
    ...
    switch (retval) {
        case 1: pdfAddOutputIntentA(pdf, LUMAS_REPO_ROOT "/sample_rgb.icc"); break;
        case 2: pdfAddOutputIntentA(pdf, LUMAS_REPO_ROOT "/sample_rgb.icc" /* production: a real CMYK press profile */); break;
        case 3: pdfAddOutputIntentA(pdf, LUMAS_REPO_ROOT "/sample_gray.icc"); break;
    }
...
int main() {
    ...
    if (ConvertFile(pdf, ctFacturX_Comfort, LUMAS_REPO_ROOT "/sample_invoice.pdf",
                    LUMAS_REPO_ROOT "/factur-x.xml", outFile))
