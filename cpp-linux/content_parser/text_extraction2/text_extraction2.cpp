...
static void WriteWChars(const LWCHAR* p, size_t count){
    if(!p || count == 0) return;
    fwrite(p, sizeof(LWCHAR), count, m_File);
}
...
static void DoWritePageIdentifier(SI32 PageNum){
    ...
    // snprintf, not swprintf: swprintf produces wchar_t, which is UTF-32 off
    // Windows and cannot go into this UTF-16LE file.
    char buf[128];
    snprintf(buf, sizeof(buf), "%%----------------------- Page %d -----------------------------\r\n", (int)PageNum);
    WriteWStr(buf);
}
...
static SI32 DoAddText(TCTM* Matrix, TTextRecordWPtr Kerning, UI32 Count, double Widen, LBOOL Decoded){
    ...
    for(UI32 i = 0; i < Count; i++){
        ...
        // rec.Text is already LWCHAR* (TTextRecordW); the old
        // (const wchar_t*) cast was the bug, not the remedy.
        WriteWChars(rec.Text, (size_t)rec.Length);
    }
